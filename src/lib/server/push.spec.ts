...
describe('push', () => {
	...
	it('moves a device to whoever subscribes it last', async () => {
		...
		await saveSubscription(db, anna.id, sub(1), 'iPhone');
		await saveSubscription(db, ben.id, sub(1), 'iPhone');
	...
	it('sends to every device and forgets devices that are gone', async () => {
		const db = testDb();
		const { owner: anna } = await seedFamily(db, 'anna');
		await saveSubscription(db, anna.id, sub(1), null);
		await saveSubscription(db, anna.id, sub(2), null);
		await saveSubscription(db, anna.id, sub(3), null);
